...
pub const Texture = extern struct {
    ...
    extern fn wlr_texture_from_pixels(renderer: *wlroots.Renderer, wl_fmt: wayland.struct_wl_shm.enum_wl_shm_format, stride: u32, width: u32, height: u32, data: ?*const c_void) ?*Texture;
    extern fn wlr_texture_from_wl_drm(renderer: *wlroots.Renderer, data: *wayland.Resource) ?*Texture;
    extern fn wlr_texture_from_dmabuf(renderer: *wlroots.Renderer, attribs: *wlroots.DmabufAttributes) ?*Texture;
    ...
    pub fn fromPixels(renderer: *wlroots.Renderer, wl_fmt: wayland.struct_wl_shm.enum_wl_shm_format, stride: u32, width: u32, height: u32, data: ?*const c_void) !*Texture {
        return wlr_texture_from_pixels(renderer, wl_fmt, stride, width, height, data) orelse error.Failure;
    }

    pub fn fromWlDrm(renderer: *wlroots.Renderer, data: [*c]wayland.Resource) !*Texture {
        return wlr_texture_from_wl_drm(renderer, data) orelse error.Failure;
    }

    pub fn fromDmabuf(renderer: *wlroots.Renderer, attribs: *wlroots.DmabufAttributes) !*Texture {
        return wlr_texture_from_dmabuf(renderer, attribs) orelse error.Failure;
    }
    ...
    pub fn writePixels(texture: *Texture, stride: u32, width: u32, height: u32, src_x: u32, src_y: u32, dst_x: u32, dst_y: u32, data: ?*const c_void) !void {
        if (!texture.wlr_texture_write_pixels(stride, width, height, src_x, src_y, dst_x, dst_y, data)) {
            return error.Failure;
        }
    }

    pub fn toDmabuf(texture: *Texture, attribs: *wlroots.DmabufAttributes) !void {
        if (!texture.wlr_texture_to_dmabuf(attribs)) {
            return error.Failure;
        }
    }
